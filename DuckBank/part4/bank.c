...
int main(int argc, char *argv[]){
    ...
    struct stat st = {0};
    if (stat("savings", &st) == -1) {
        mkdir("savings", 0700); // 0700 grants full permissions to the owner
    }
    ...
    pthread_t puddlesBankThread;
    pthread_create(&puddlesBankThread, NULL, puddlesBanker, (void *)&numAccounts);
    ...
    pthread_join(puddlesBankThread, NULL); 
    close(pipe_fd[1]);
...
void accountInit(int numAccounts, account *accounts, FILE *file){
    ...
    int shm_fd = shm_open(SAVINGS_FILE, O_CREAT | O_RDWR | O_TRUNC, 0666);
    if (shm_fd == -1) {
        perror("Error opening shared memory");
        exit(1);
    }
    
    if (ftruncate(shm_fd, numAccounts * sizeof(account)) == -1) {
    ...
    shared_savings = mmap(NULL, numAccounts * sizeof(account), PROT_READ | PROT_WRITE, MAP_SHARED, shm_fd, 0);
    
    if (shared_savings == MAP_FAILED) {
    ...
    for(int i = 0; i < numAccounts; i++){
        ...
        shared_savings[i].balance = accounts[i].balance * 0.2;
        shared_savings[i].reward_rate = 0.02; // Flat reward rate of 2%
        shared_savings[i].transaction_tracter = 0.0;
...
void *puddlesBanker(void *arg) {
    int numAccounts = *(int *)arg;
    while (1) {
        for (int i = 0; i < numAccounts; i++) {
            pthread_mutex_lock(&workerLock);
            shared_savings[i].balance += shared_savings[i].transaction_tracter * shared_savings[i].reward_rate;
            shared_savings[i].transaction_tracter = 0.0;
...
void applyRewards(account *accounts, int numAccounts){
    ...
    for(int i = 0; i < numAccounts; i++){
        ...
        shared_savings[i].transaction_tracter = accounts[i].transaction_tracter;
...
void printBalance(account *accounts, int numAccounts){
    ...
    FILE *savingsOutput = fopen("savings/output.txt", "w");
    
    if (output == NULL || savingsOutput == NULL) {
        perror("Error creating output file!");
        return;
    }

    for (int i = 0; i < numAccounts; i++) {
        // Print Duck Bank balance
        fprintf(output, "Account %d balance (Duck Bank):\t%.2f\n", i, accounts[i].balance);
        
        // Print Puddles Bank savings balance
        fprintf(savingsOutput, "Account %d balance (Puddles Bank):\t%.2f\n", i, shared_savings[i].balance);
    }

    fclose(output);
    fclose(savingsOutput);
}
